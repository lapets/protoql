    ...
    */
      ...
      (function(raw) {
        ...
        var set =
          (function(xs) {
            ...
            for (var i = 0; i < xs.length; i++) {
              var add = true; for (var j = 0; j < ys.length; j++) if (ys[j] == xs[i]) add = false;
              if (add) ys.push(xs[i]);
            }
            return ys;
          });
        raw =
          raw
            ...
            .replace(/</g, "(").replace(/>/g, ")")
            .replace("graph[", "[");
        var nodes_links = eval(raw), nodes = nodes_links[0], links = nodes_links[1];
        ...
        for (var i = 0; i < nodes.length; i++) {
          ...
          for (var j = 0; j < links.length; j++) {
            if (links[j][0] == nodes[i]) links[j][0] = [row, col];
            if (links[j][1] == nodes[i]) links[j][1] = [row, col];
          }
          listOfListsOfNodes[row].push(nodes[i]);
          col = i % cols;
          row = row + (i % cols == 0) ? 1 : 0;
        }
        //console.log(listOfListsOfNodes);
    ...
    Notation.relation =
      (function(raw) {
        ...
        for (var i = 0; i < nodes.length; i++) {
          for (var j = 0; j < links.length; j++) {
            if (links[j][0] == nodes[i]) links[j][0] = [i, 0];
            if (links[j][1] == nodes[i]) links[j][1] = [i, 2];
          }
          listOfListsOfNodes.push([nodes[i], null, nodes[i]]);
        }
        return Notation.listOfListsOfNodesToData({nodes:listOfListsOfNodes, links:links, groups:[]}, false);
      });
