      ...
      (function(d, deriveLinks) {
        ...
        for (var i = 0; i < d.links.length; i++) {
          ...
          if (d.links[i].length == 3)
            e.label = d.links[i][2];
          Visualization.data.links.push(e);
        }
  ...
  */
  ...
  protoql.Visualization = (function(arg) {
    ...
    */
    ...
    Visualization.zoompan = {translate:{x:0, y:0}};
        ...
        Visualization.linksEnter
          .attr("d", function (e) {
            ...
            if (G.inShape(t, e.source) || G.inShape(s, e.target)) {
              ...
              return lineFunction([s, G.offpath(s, t, 5), t]);
            } else {
              return lineFunction([s, t]);
            }
          })
          .each(function(e) {
            //G.scale(Visualization.zoompan.scale, Visualization.zoompan.translate);
            var v = G.sum(G.offpath(e.source, e.target, 15), Visualization.zoompan.translate);
            e.labelText.attr("transform", function (d) { return "translate("+v.x+","+v.y+")"; });
          });
      });
          ...
          canvas.call(
              d3.behavior.zoom()
                .on("zoom", function() {
                  ...
                  Visualization.zoompan = {translate:{x:tr[0], y:tr[1]},scale:sc};
                  Visualization.vis.attr("transform", "translate(" + tr + ")" + " scale(" + sc + ")");
                })
            )
