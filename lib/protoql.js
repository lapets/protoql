    ...
    Visualization.build =
      ...
      (function() {
          ...
          for (var col = 0; col < inp.nodes[row].length; col++) {
            if (inp.nodes[row][col] != null) {
              ...
              var node = {
                  ...
                  rx: 5, ry: 5, offx: 0, offy: 0, x: Visualization.layout.grid.width*col, y: Visualization.layout.grid.width*row
                }
        ...
        for (var i = 0; i < inp.links.length; i++) {
          ...
          Visualization.data.links.push({source:inp.nodes[s[0]][s[1]], target:inp.nodes[t[0]][t[1]]});
        }
